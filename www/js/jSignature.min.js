...
p)*e),j=this.canvasContext,e=f.x,p=f.y,h=d.x,x=d.y,B=f.x+k.x,f=f.y+k.y;k=d.x+q.x;q=d.y+q.y;j.beginPath();j.moveTo(e,p);j.bezierCurveTo(B,f,k,q,h,x);j.stroke()}}c.getLength()<=this.lineCurveThreshold&&(c=this.canvasContext,f=d.x,d=d.y,k=g.x,g=g.y,c.beginPath(),c.moveTo(f,d),c.lineTo(k,g),c.stroke())},y=function(b){var a=b.x.length-1;if(0<a){var d=new m(b.x[a],b.y[a]),g=new m(b.x[a-1],b.y[a-1]),c=g.getVectorToPoint(d);if(c.getLength()>this.lineCurveThreshold){if(1<a){b=(new m(b.x[a-2],b.y[a-2])).getVectorToPoint(g);
    var f = (new n(b.x + c.x, b.y + c.y)).resizeTo(c.getLength() / 2), c = this.canvasContext; b = g.x; var a = g.y, j = d.x, k = d.y, e = g.x + f.x, g = g.y + f.y, f = d.x, d = d.y; c.beginPath(); c.moveTo(b, a); c.bezierCurveTo(e, g, f, d, j, k)
} else c = this.canvasContext, b = g.x, g = g.y, a = d.x, d = d.y, c.beginPath(), c.moveTo(b, g), c.lineTo(a, d); c.stroke()
}
}
}; s.prototype.resetCanvas = function (b) {
    var a = this.canvas, d = this.settings, c = this.canvasContext, f = this.isCanvasEmulator, j = a.width, k = a.height; c.clearRect(0, 0, j + 30, k + 30); c.shadowColor = c.fillStyle = d["background-color"]; c.fillStyle = '#ffffff';c.fillRect(0, 0, j + 30, k + 30);
...
String(a)+"'");return this},d=new w,c=e,j,k=function(){d.publish(c+".parentresized")};$(p).bind("resize."+c,function(){j&&clearTimeout(j);j=setTimeout(k,500)}).bind("mouseup."+c,function(){d.publish(c+".windowmouseup")});var x={},B={"default":function(){return this.toDataURL("image/jpeg")},"native":function(b){return b},image:function(){var b=this.toDataURL();if("string"===typeof b&&4<b.length&&"data:"===b.slice(0,5)&&-1!==b.indexOf(",")){var a=b.indexOf(",");return[b.slice(5,a),b.substr(a+1)]}return[]}},q={"native":function(b,
